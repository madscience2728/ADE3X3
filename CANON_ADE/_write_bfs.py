"""Helper: write axiom_bfs.py with proper UTF-8 encoding."""
import os, pathlib
...
def build_canon() -> OCS:
    \"\"\"C4 baked into orbit basis. Add C2 (r-blindness) on top.\"\"\"
    cs = OCS()
    enc_rblind(cs)
    return cs
...
class AxiomState:
    ...
    def impose(self, ax: str) -> Optional["AxiomState"]:
        ...
        if enc:
            enc(ncs)
        if not ncs.is_consistent():
            return None
        return AxiomState(self.imposed | {ax}, ncs)
...
if __name__ == "__main__":
    ...
    print(f"\\nFull summary ({len(res)} consistent combos):")
    for s in res[:20]:
        print(f"  {s.summary()}")
    print("\\nNear-exact (d.o.f. <= 10):")
    for s in res:
        d = s.dof()
        if 0 <= d <= 10:
            print(f"  d={d}: {sorted(s.imposed)}")
"""

dest = pathlib.Path("CANON_ADE/axiom_bfs.py")
