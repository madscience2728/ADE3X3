...
EXPORT_ARTIFACTS = {
    ...
    'Higher-Arity Schema Exports': [
        ...
        'exports/schema_AXXC_summary.md', 'exports/bridge_AXXC.csv',
    ],
    'Arity-4 Interaction Inventories': [
        'exports/CXXC_face_inventory.csv', 'exports/CXXC_face_inventory.md',
        'exports/CXXC_face_patterns.csv', 'exports/CXXC_face_patterns.md',
        'exports/CXXC_xx_marginals.csv', 'exports/CXXC_xx_marginals.md',
        'exports/CXXC_left_CXC_marginals.csv', 'exports/CXXC_right_CXC_marginals.csv',
        'exports/CXXC_cxc_marginals.md',
        'exports/CXXC_joint_CXC_XX_CXC.csv', 'exports/CXXC_joint_CXC_XX_CXC.md',
        'exports/AXXC_face_inventory.csv', 'exports/AXXC_face_inventory.md',
    ],
}
...
def generate():
    ...
    w()
    w(f"Generated: {ts}")
    w("Generator: ade3x3_step13e_update_md_dossier_generator.py")
    w("Source: Current warehouse state (steps 1-31, repairs 10b/13b, exports 14-31)")
    w()
    ...
    w(f"**Generated:** {ts}")
    w("**Generator Script:** ade3x3_step13e_update_md_dossier_generator.py")
    w("**Provenance:** Built from steps 1-31, with orbit metadata repair (step 10b)")
    ...
    w("| AXXC   | 4           | 6         | 531,441 | -      | Yes     | second arity-4 schema |")
    w()
    ...
    w("CXXC and AXXC are the first typed schemas beyond the arity-3 core.")
    ...
    w("- First two arity-4 typed schemas (CXXC, AXXC) registered and bridged")
    w("- CXXC face inventory, face-pattern summary, and joint interior co-occurrence export")
    w("- CXXC marginals conditioned on XX and both CXC faces")
    w("- AXXC face inventory export")
    ...
    w()
    w("### Arity-4 Interior Determination in CXXC")
    w()
    w("[EXACT_DERIVED] / [MEASURED_FROM_CODE]")
    w()
    w("- CXXC full population = 531,441 rows")
    w("- Distinct 6-face orbit-patterns = 2,744")
    w("- Distinct joint keys (left CXC, middle XX, right CXC) = 2,744")
    w("- Therefore the interior 3-tuple (left CXC orbit, middle XX orbit, right CXC orbit)")
    w("  fully determines the exported 6-face orbit-pattern for CXXC at the current layer")
    w("- Pattern multiplicities range from 27 to 216")
    w()
    w("### Arity-4 Marginal Inventories")
    w()
    w("[MEASURED_FROM_CODE]")
    w()
    w("- CXXC conditioned on middle XX orbit: 56 rows, counts 2,187–17,496, distinct face-patterns 25–81")
    w("- CXXC conditioned on left CXC orbit: 50 rows, counts 2,187–17,496, distinct face-patterns 20–81")
    w("- CXXC conditioned on right CXC orbit: 50 rows, counts 2,187–17,496, distinct face-patterns 20–81")
    w("- AXXC face inventory exported: full population 531,441 rows with faces AX, XX, XC, AC, left AXC, right AXC")
    ...
    w("[OPEN_FRONT]")
    w()
    w("- First two arity-4 typed schemas exist (CXXC, AXXC); broader higher-arity typed schema")
