...
## Proof Skeleton For The Remaining Gap

The clean next target is not to attack `Delta subset span(H)` directly, but to prove the equivalent kernel-spanning statement for `H`.

### Lemma chain

1. Part 1 gives `rank([Sigma | H | Delta]) = R` for every minimum-rank decomposition.
2. Since `rank(Gamma) = 9`, it follows that `dim(ker Gamma) = R - 9`.
3. `Gamma` annihilates both `H` and `Delta`, so `col([H | Delta]) subset ker(Gamma)`.
4. The valid decomposition computations already realize equality:
  - `rank([H | Delta]) = R - 9`
  - therefore `col([H | Delta]) = ker(Gamma)`.
5. Hence
  - `Delta subset span(H)`
  - iff `col(H) = col([H | Delta])`
  - iff `col(H) = ker(Gamma)`
  - iff `rank(H) = R - 9`.

So the conservation law has reduced to a single linear-algebra statement:

> for every minimum-rank multiplication decomposition, the eta-columns span all of `ker(Gamma)`.

### Per-channel right-inverse structure

For each live channel `s in {0,1,2}`, the multiplication tensor equations imply

`Gamma * P_s = I_9`

where `P_s` is the `R x 9` matrix of live products `alpha_k[:,s] tensor beta_k[s,:]`.

Fix any particular solution `X_0` of `Gamma * X = I_9`. Then each channel can be written as

`P_s = X_0 + K_s`

with every column of `K_s` in `ker(Gamma)`. The nuisance blocks are exactly differences of these kernel lifts:

- `Eta1 = P_0 - P_1 = K_0 - K_1`
- `Eta2 = P_1 - P_2 = K_1 - K_2`

Thus `H` is generated by the difference sector of three right inverses of the same map `Gamma`.

### Main route to push

The natural theorem to prove next is:

> If `(P_0, P_1, P_2)` comes from a valid minimum-rank `3x3` matrix-multiplication decomposition and `Gamma * P_s = I_9` for each `s`, then
> `span(col(P_0 - P_1), col(P_1 - P_2)) = ker(Gamma)`.

This is stronger and cleaner than phrasing the target as `Delta subset span(H)`, because it isolates the only missing step as a spanning theorem inside the kernel of `Gamma`.

### Why route (b) is the right route

The genericity heuristic is informative but insufficient for a theorem. Generic `18`-vector spanning in a `14`-dimensional kernel only shows that the bad locus `rank(H) < R - 9` should be lower-dimensional; it does not rule out exceptional valid decompositions.

Likewise, irreducibility of the decomposition variety would not by itself eliminate a proper closed exceptional locus. A universal theorem needs direct structure from the multiplication equations.

That structure is the channelwise identity `Gamma * P_s = I_9`. The anticommutator counterexample sits outside this regime, which is exactly why its `H` block fails to saturate `ker(Gamma)`.

### Candidate proof program

1. Express every valid decomposition through the triple of right inverses `(P_0, P_1, P_2)`.
2. Identify the linear constraints on the kernel lifts `(K_0, K_1, K_2)` imposed by the fact that they come from factorized rank-1 terms, not arbitrary right inverses.
3. Show that any nonzero functional on `ker(Gamma)` vanishing on both `col(P_0 - P_1)` and `col(P_1 - P_2)` would force an inadmissible extra linear dependence among the live-product profiles.
4. Conclude that the orthogonal complement of `col(H)` inside `ker(Gamma)` is zero, hence `col(H) = ker(Gamma)` and `rank(H) = R - 9`.

The representation-theoretic viewpoint is still useful, but as a tool inside this route: the cyclic relation among the three channels should clarify which kernel directions belong to the difference sector and which cannot survive in a valid multiplication decomposition.