...
ROTATION_CAPS = [cap / 2 for cap in range(4, 29)]
BUDGET_LIMITS = [0.02, 0.05, 0.1, 0.2, 0.4, 0.8, 1.2, 2.0, 3.0]
MAX_STEPS = 24
GAIN_TOL = 1e-9
...
def build_policy(rotation_cap_deg: float, total_budget: float) -> dict[str, object]:
    ...
    return {
        'policy_name': f'sharpen_rot{rot_label}_budget{budget_label}',
        'max_steps': MAX_STEPS,
...
def build_delta_rows(frontier_rows: list[dict[str, object]]) -> list[dict[str, object]]:
...
def main() -> None:
    ...
    delta_rows = build_delta_rows(frontier_rows)

    write_csv(
        OUT_DIR / 'boundary_sharpening_grid.csv',
        grid_rows,
        [
    ...
    write_csv(
        OUT_DIR / 'boundary_sharpening_frontier.csv',
        frontier_rows,
        [
    ...
    write_csv(
        OUT_DIR / 'boundary_sharpening_deltas.csv',
        delta_rows,
        [
    ...
    write_json(
        OUT_DIR / 'boundary_sharpening_scan.json',
        {
    ...
    print('Phase 32 boundary-sharpening scan complete.')
