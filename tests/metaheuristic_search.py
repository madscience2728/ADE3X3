...
def _run_optimizer(db_path: Path, seed_dir: Path, max_generations: int, config_overrides: dict | None = None):
    ...
    from db_optimizer.cpu_refine import cpu_minimax_refine, cpu_pair_refine, cpu_lbfgs_refine, cpu_algebraic_snap, cpu_full_refine, cpu_als_init, cpu_slp_refine
    ...
    # ── Basin registry: track structurally distinct basins ──
    from db_optimizer.basin_fingerprint import BasinRegistry, compute_basin_fingerprint
    _basin_registry = BasinRegistry()
            ...
            while _is_running():
                ...
                for island, n_children in allocations.items():
                    ...
                    children = generate_batch(
                        ...
                        p_teleport=icfg.get("p_teleport", 0.0),
                        p_als_restart=icfg.get("p_als_restart", 0.0),
                ...
                try:
                    if kind == "screen":
                        ...
                        if n_surv > 0:
                            ...
                            # Batch minimax to avoid OOM on large survivor sets
                            MM_CHUNK = min(4096, GPU_MINIMAX_BATCH)
                            minimax_results = []
                            for chunk_start in range(0, n_surv, MM_CHUNK):
                                chunk_end = min(chunk_start + MM_CHUNK, n_surv)
                                chunk_res = gpu_minimax_refine_stacked(
                                    s_alpha[chunk_start:chunk_end],
                                    s_beta[chunk_start:chunk_end],
                                    s_gamma[chunk_start:chunk_end],
                                    sweeps=GPU_MINIMAX_SWEEPS,
                                    n_trials=GPU_MINIMAX_N_TRIALS,
                                    fine_range=GPU_MINIMAX_FINE_RANGE,
                                )
                                minimax_results.extend(chunk_res)
                            ...
                            for i, (a_r, b_r, g_r, fit) in enumerate(minimax_results):
                                ...
                                # Register with basin fingerprint tracker
                                try:
                                    bp = compute_basin_fingerprint(a_r, b_r, g_r)
                                    is_novel, novelty = _basin_registry.register(
                                        s_ids[i], bp, fit,
                                        generation=_gen_counter[0],
                                        wall_time=time.time(),
                                        blob=mm_blobs[-1],
                                    )
                                    if is_novel and fit < 0.5:
                                        print(f"  [BASIN] NEW basin discovered! fp={bp.coarse_key[:8]} "
                                              f"fit={fit:.6f} (total basins: {_basin_registry.n_basins})")
                                except Exception:
                                    pass  # fingerprinting is advisory, never block pipeline
                        ...
                        minimax_results = []
                        MM_CHUNK2 = min(4096, GPU_MINIMAX_BATCH)
                        for chunk_start in range(0, N, MM_CHUNK2):
                            chunk_end = min(chunk_start + MM_CHUNK2, N)
                            chunk_res = gpu_minimax_refine_stacked(
                                alpha_np[chunk_start:chunk_end],
                                beta_np[chunk_start:chunk_end],
                                gamma_np[chunk_start:chunk_end],
                                sweeps=GPU_MINIMAX_SWEEPS,
                                n_trials=GPU_MINIMAX_N_TRIALS,
                                fine_range=GPU_MINIMAX_FINE_RANGE,
                            )
    ...
    try:
        ...
        while _is_running():
            ...
            if migration_counter >= 5:
                ...
                # 3) Basin-novelty injection: seed underexplored basins into exploit islands
                underexplored = _basin_registry.get_underexplored_basins(
                    min_fitness=TIER1_SURVIVOR_THRESHOLD, max_pop=3)
                if underexplored:
                    # Pick exploit-tier islands to receive novel basin seeds
                    exploit_islands = [m for m in ISLAND_LAYOUT
                                       if m["role"] in ("exploit", "strong_exploit", "balanced")]
                    if exploit_islands:
                        for basin_rec in underexplored[:5]:  # inject up to 5 underexplored basins
                            if basin_rec.representative_blob is None:
                                continue
                            target_island = _rng.choice(exploit_islands)
                            alpha, beta, gamma = blob_to_factors(basin_rec.representative_blob)
                            # Light perturbation to create diversity within the basin
                            inject_rng = np.random.default_rng()
                            rows = []
                            for _ in range(min(3, target_island["cap"] // 10)):
                                a_p = alpha + inject_rng.normal(0, 0.005, alpha.shape)
                                b_p = beta + inject_rng.normal(0, 0.005, beta.shape)
                                g_p = gamma + inject_rng.normal(0, 0.005, gamma.shape)
                                blob = factors_to_blob(a_p, b_p, g_p)
                                s_hash = compute_support_hash(a_p, b_p, g_p)
                                s_sig = compute_support_sig(a_p, b_p, g_p)
                                rows.append((blob, s_hash, s_sig, "basin_inject",
                                             target_island["id"], gen))
                            if rows:
                                _db_submit("insert_candidates", (rows,))
                        if underexplored:
                            print(f"  [BASIN] Injected {len(underexplored)} underexplored basins "
                                  f"into exploit islands")
            ...
            _db_submit("log_event", ("gen_complete", gen,
                                     json.dumps({
                                         ...
                                         "basins": _basin_registry.to_dict(),
                                     }), wall))
            _set_activity("monitor", "idle")
            ...
            if gen % 5 == 0:
                basin_info = f"basins={_basin_registry.n_basins}"
                top_basins = _basin_registry.get_top_basins(3)
                if top_basins:
                    basin_info += " top=[" + ", ".join(
                        f"{r.fingerprint[:6]}:{r.best_fitness:.6f}(n={r.population})"
                        for r in top_basins
                    ) + "]"
                print(f"[MON] gen={gen} total={total} pending={pend} best={best_str} queue={qsize} | {isl_str}")
                print(f"  [MON] {basin_info}")
